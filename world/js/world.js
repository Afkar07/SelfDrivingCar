class World {
   ...
   draw(ctx, viewPoint, showStartMarkings = true, renderRadius = 2500) {
      this.#updateLights();

      for (const env of this.envelopes) {
         env.draw(ctx, { fill: "DimGray", stroke: "DimGray", lineWidth: 15 });
      }
      ...
      for (const seg of this.graph.segments) {
         seg.draw(ctx, { color: "white", width: 4, dash: [50, 50] });
      }
